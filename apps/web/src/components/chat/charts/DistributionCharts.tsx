    ...
    <ChartShell title={plan.title}>
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={bins} margin={{ top: 8, right: 16, left: 8, bottom: 4 }}>
          ...
          <XAxis dataKey="x" tick={{ fill: TICK, fontSize: 10 }} interval={0} angle={-25} textAnchor="end" height={50} />
    ...
    <ChartShell title={plan.title}>
      <ResponsiveContainer width="100%" height={240}>
        <ScatterChart margin={{ top: 8, right: 16, left: 8, bottom: 4 }}>
          <CartesianGrid strokeDasharray="3 3" stroke={GRID} />
          <XAxis type="number" dataKey="x" name={sx?.label} tickFormatter={formatAxisInr} tick={{ fill: TICK, fontSize: 11 }} />
          <YAxis type="number" dataKey="y" name={sy?.label} tickFormatter={formatAxisInr} tick={{ fill: TICK, fontSize: 11 }} />
